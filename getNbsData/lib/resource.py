...
class Resource:
  ...
  def get(self,url,params):
    if params == "":
      ...
      q = "?"
    url = url + self.ext + q + rest.parse.urlencode(params)
    #print repr(rest.urlopen(url).geturl( ))
    return rest.request.urlopen(url).read()

  # HTTP POST
  def post(self,url,data):
    url = url + self.ext
    data = rest.parse.urlencode(data).encode("utf-8")
    # print repr(data)
    # print repr(rest.urlopen(url,data).geturl( ))
    return rest.request.urlopen(url,data).read()
